...
const ExtractedInfoDisplay: React.FC<ExtractedInfoDisplayProps> = ({
  ...
  // テーブルセルの更新
  const updateTableCell = (fieldName: string, rowIndex: number, columnName: string, value: any) => {
    const updatedTable = [...(editedInfo[fieldName] || [])];
  ...
  const renderField = (field: Field) => {
    // 古いスキーマ形式のサポート
    if (field.type === 'table' && 'columns' in field) {
              ...
              {editMode ? (
                <div className="relative">
                  <input
                    ...
                    className="w-full p-2 border border-gray-300 rounded"
                    onFocus={() => onHighlightField(`${field.name}.${subField.name}`, true)}
                  />
                  <button
                    type="button"
                    onClick={() => onHighlightField(`${field.name}.${subField.name}`, true)}
                    className="absolute right-2 top-2 text-blue-500 hover:text-blue-700"
                  >
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
  ...
  // 旧形式のテーブルフィールドの表示（後方互換性のため）
  const renderTableField = (field: Field) => {
    if (!field.columns) return null;
    
    const tableData = editMode ? editedInfo[field.name] || [] : extractedInfo[field.name] || [];
                    ...
                    <td key={column.name} className="px-6 py-4 whitespace-nowrap">
                      {editMode ? (
                        <input
                          ...
                          onChange={(e) => updateTableCell(field.name, rowIndex, column.name, e.target.value)}
                          className="w-full p-1 border border-gray-300 rounded"
                          onFocus={() => onHighlightCell(field.name, rowIndex, column.name)}
                        />
                      ) : (
                        <div 
                          className="text-sm text-gray-900 cursor-pointer hover:bg-blue-50 p-1 rounded"
                          onClick={() => onHighlightCell(field.name, rowIndex, column.name)}
                        >
                          {row[column.name] || ''}
                        </div>
                      )}
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <button
                        ...
                        onClick={() => {
                          ...
                          updateFieldValue(field.name, updatedTable);
                        }}
                        className="text-red-600 hover:text-red-900"
                      >
                        削除
        ...
        {editMode && (
          <button
            ...
            onClick={() => {
              ...
              updateFieldValue(field.name, [...tableData, newRow]);
            }}
            className="mt-2 text-blue-600 hover:text-blue-800"
          >
            + 行を追加
