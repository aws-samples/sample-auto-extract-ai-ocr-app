...
export class Waf extends Construct {
  ...
  constructor(scope: Construct, id: string, props: WafProps) {
    ...
    // IPアドレス制限（IPv4）
    if (props.options.allowedIpV4AddressRanges && props.options.allowedIpV4AddressRanges.length > 0) {
    ...
    // IPアドレス制限（IPv6）
    if (props.options.allowedIpV6AddressRanges && props.options.allowedIpV6AddressRanges.length > 0) {
    ...
    const webAcl = new wafv2.CfnWebACL(this, 'WebAcl', {
      ...
      scope: 'CLOUDFRONT', // CloudFront用はグローバルスコープ
      defaultAction: {
        allow: {},
      },
      visibilityConfig: {
