...
- **コメントは WHY を書く**: 何をするか（HOW/WHAT）の逐語説明でなく、なぜそうするかを書く。
- **変更履歴・before/after は書かない**: 旧実装を前提にした説明（「以前は〜」「〜に変更」）は、その前提を知らない読み手には意味が通らず、git 履歴と二重になって古くなる。コメントは現在の実装だけを説明する。
